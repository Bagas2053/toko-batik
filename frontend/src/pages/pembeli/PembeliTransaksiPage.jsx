...
export default function PembeliTransaksiPage() {
  ...
  async function bayarTerpilih() {
    ...
    for (const id of ids) {
      try {
        ...
        await pembeliApi.updatePembelian(id, {
          ...
          pembayaran: 'Dibayar',
          catatan: buildCatatanBayar(row, metodeMassal),
        });
        ok += 1;
      } catch {
        fail += 1;
    ...
    setSelected(new Set());
    reload();
    if (ok) setMsg(`Berhasil bayar ${ok} pesanan (${labelBayar(metodeMassal)}).`);
  ...
  async function bayarSemuaBelum() {
    ...
    if (!window.confirm(`Bayar semua ${belumBayarList.length} pesanan yang belum dibayar?`)) return;
    setBusyAll(true);
    setMsg('');
    setErr('');
    let ok = 0;
    let fail = 0;
    for (const p of belumBayarList) {
      try {
    ...
    setSelected(new Set());
    reload();
    if (ok) setMsg(`Berhasil bayar semua: ${ok} pesanan.`);
    if (fail) setErr(`${fail} pesanan gagal dibayar.`);
    setBusyAll(false);
  }
